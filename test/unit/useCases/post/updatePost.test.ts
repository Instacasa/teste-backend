...
describe("Update Post", () => {
  ...
  test("Shouldn't update post title to empty/null", async () => {
    ...
    try {
      ...
      expect((error as Error).message).toEqual(
        "O título da publicação é obrigatório"
      );
    }
  });
  ...
  test("Shouldn't update post if user isn't the owner", async () => {
    ...
    const newUser = await userRepository.create(
      new User({ name: "new User", isAdmin: false })
    );
    const createPost = new CreatePost();
    const updatePost = new UpdatePost();
    const partialPost: Partial<PostInterface> = {
    ...
    try {
      ...
      expect(error as Error).toBeInstanceOf(ValidationError);
      expect((error as Error).message).toEqual(
        "Apenas o autor pode editar a publicação"
      );
    }
  });
  ...
  test("Shouldn't allow update if post has comments", async () => {
    ...
    const newComment = await commentRepository.create(
      new Comment({ text: "new comment", user: newUser, post })
    );
    try {
      ...
      expect(error as Error).toBeInstanceOf(ValidationError);
      expect((error as Error).message).toEqual(
        "Publicações já comentadas não podem ser editadas"
      );
    }
  });
});
